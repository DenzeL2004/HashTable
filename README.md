...
- Среднеквадратичное размеров списков: 2194.7
...
2. Все тесты проводились с одними и теми же наборами данных: [words.txt](resource/words.txt), [test.txt](resource/test.txt).
3. Компьютер был подключен к сети, чтобы система не снимала нагрузку с CPU для экономии заряда.
...
6. Программа тестировалась на ноутбуке с процессором Intel(R) Core(TM) i5-10300H CPU @ 2.50GHz.
...
В дальнейшем каждая оптимизация будет тестироваться 20 раз после чего в результат будет идти среднее время. Замеры будут произведены, по относительному времени работы теста.
...
Среднее время работы: $287.61 \pm 2.65$
...
<h4> 1. Увеличение размера хеш таблицы </h4>
При первом измерении размер хеш-таблицы был равен 700, а средняя длина списка 80. Увеличим размер таблицы до 10000 и зафиксируем результат.


||Среднее время работы, мс|Процент ускорения|
|:------------|:------------------:|:---------------:|
|Относительное время работы |$104.45 \pm 1.6$  |    $ 74.4 $      |


Скорость работы программы увеличилась, так как средний размер размер списков стал равен 7, что уменьшило время поиска.


В дальнейшем каждая оптимизация будет сравниваться относительно времени, достигнутого при увеличении буфера.


---


<h5> 2. Ускорение хеш-функции </h5>


По результатам анализа работы профайлера ([профилирование 1.1](resource/img/simpleVersionProf.png)) было выявлено, что более весомый вклад в замедление производительности программы вносит хеш-функция - **CRC32Hash**.


|![Профилирование 1.1](resource/img/upperCapacity.png)|
|:--------------------------------------------------------|
|Профилирование 1.1|
...
<h5> 3.1 Оптимизация ассемблерной вставки </h5>
