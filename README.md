...
В дальнейшем каждая оптимизация будет тестироваться 20 раз после чего в результат будет идти среднее время. Каждая оптимизация будет сравниваться относительно первоначальной (неоптимизированной) версии.
...
Запустим [callgrind](https://valgrind.org/docs/manual/cl-manual.html) для профилирования программы.
...
После применение методов оптимизаций, то, что еще можно было бы ускорить - это функция **CRC32Hash**. Функции ```GetText``` является временем затратной, однако ее вызов происходит не часто, поэтому ее временем можно пренебречь.
...
- Подтверждаем размышления о том, что не всегда флагов компилятора достаточно для оптимизации программы.
