...
2. Все измерения производились с опцией -O3 компилятора gcc.
...
Среднее время работы: $2036 \pm 2$ мс.
...
По результатам анализа работы профайлера ([профилирование 1.1](resource/img/BaseProf.png) было выявлено, что более весомый вклад в замедление производительности программы вносит хеш-функция - **CRC32Hash**.


|![Профилирование 1.1](resource/img/BaseProf.png)|
|:--------------------------------------------------------|
|Профилирование 1.1|


Можем заметить, что благодаря строгим требованиям к входным данным, а именно файл состоит из английских слов, длина которых не превышает 32 символов, воспользуемся ассемблерной вставкой. Чтобы получить максимальную выгоду от использования инструкций выровняем адреса слов к кратным 32.
...
![Fastcrc32](resource/img/asmCRC32.png)
...
Из данных профидирования файла мы видем, что общей процент времени, затрачиваемый на функцию поиска хеша строки, стал занимать меньше пяти процентов. 
...
||Среднее время работы, мс|Процент абсолютного ускорения |Процент относителього ускорения|
|:---------------|:------------------:|:---------------:|:------:|
|Относительное время работы|$1734 \pm 3$   |    $15$      | $15$ |
...
По результатам анализа работы профайлера ([профилирование 1.2](resource/img/FastCrc32Prof.png)), заметим, что после оптимизации функции поиска хеша строки, следующей для ускорения является функция сравнение строк.
...
<h5>  Выравнивание буфера. Использование интринсик команд </h5>
...
|![Профилирование 2.1](resource/img/FastStrcmProf.png)|
|:--------------------------------------------------------|
|Профилирование 2.1|



Из результатов профилирования видим, что функция сравнения строк испольняться быстрее почти в два раза стала.
...
||Среднее время работы, мс|Процент абсолютного ускорения |Процент относителього ускорения|
|:---------------|:------------------:|:---------------:|:------:|
|Относительное время работы|$1401 \pm 2$   |     $31.1$      | $19.2$ |
...
<h5> 3 Ассемблерная функция. </h5>


Из последнего результата профилирования ([Профилирование 2.1](resource/img/FastStrcmProf.png)) ускорим
функцию поиска элемента в списке, переписав ее на ассемблере и, сдалав функцию сравнения строк **inline**.
...
![MySolution](resource/img/NasmFindList.png)|


|![Профилирование 3.1](resource/img/resultsProf.png)|
|:--------------------------------------------------------|
|Профилирование 3.1|
...
||Среднее время работы, мс|Процент абсолютного ускорения |Процент относителього ускорения|
|:---------------|:------------------:|:---------------:|:------:|
|Относительное время работы |$1311 \pm 2$ |     $35.5$      |     $6.4$      |



Относительный процент ускорения меньше десяти процентов, поэтому процесс оптимизации было решено прекратить.
